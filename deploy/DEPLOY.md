...
Запускать его — `bash ~/anon/deploy/update.sh`. Не `./deploy/update.sh` и не
`chmod +x`: бит исполнения git отслеживает, `chmod` становится локальным
изменением файла, и следующий `git pull --ff-only` отказывается работать.
Вообще ничего не правьте и не коммитьте в развёрнутой копии — это каталог для
запуска, а не для разработки; коммит здесь разводит ветки, и `--ff-only` не
проедет уже никогда. Локальные изменения отбрасываются через
`git checkout -- .`.
