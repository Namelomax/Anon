...
def is_noise_span(text: str, label: str) -> bool:
    ...
    if is_timecode(s):  # тайм-код — не ПДн в любой метке
        return True
    # Одиночный символ и голое 1-2-значное число — не ПДн НИ ПОД КАКОЙ меткой.
    # Раньше эта защита жила только в is_stopword_entity, т.е. срабатывала лишь
    # для _SOFT_LABELS. Форматные метки её не получали, и номер пункта «1»,
    # ошибочно помеченный LLM как PASSPORT, проходил все фильтры — а
    # mask_all_occurrences размножал его по каждому вхождению: в договоре
    # купли-продажи земельного участка это дало 127 подстановок, вся нумерация
    # разделов превратилась в «[PASSPORT_1].[PASSPORT_2]. Цена…».
    # Порог 4 цифры. Сначала стояло 3, и на договоре авто вылез тот же баг на
    # цифру длиннее: «000» под меткой ADMIN_CODE маскировалось во всех
    # вхождениях и РАЗРЫВАЛО более крупные значения — госномер превращался в
    # «А [ADMIN_CODE_1] АА 00» (остаток утекал), пробег — в «22 [ADMIN_CODE_1]
    # км». Голое 1-3-значное число само по себе никого не идентифицирует:
    # настоящие реквизиты (индекс 6, БИК 9, счёт 20, год 2014) длиннее, а
    # значения с разделителями («777-003») сюда не попадают — s.isdigit() их
    # не считает цифровыми.
    if len(s) < 2:
        return True
