...
# ОКТМО — 8 или 11 цифр (по ключевому слову).
OKTMO = RegexDetector(
    "OKTMO",
    r"(?<![А-Яа-яЁёA-Za-z])ОКТМО(?![А-Яа-яЁёA-Za-z])" + _GAP + r"(\d{8}|\d{11})(?!\d)",
    group=1,
)

# ОКАТО — 8 или 11 цифр (по ключевому слову).
OKATO = RegexDetector(
    "OKATO",
    r"(?<![А-Яа-яЁёA-Za-z])ОКАТО(?![А-Яа-яЁёA-Za-z])" + _GAP + r"(\d{8}|\d{11})(?!\d)",
    group=1,
)
