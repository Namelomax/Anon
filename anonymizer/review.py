...
_RECALL_LABELS = frozenset({
    ...
    "PHONE", "EMAIL", "URL", "IP_ADDRESS", "AMOUNT",
    ...
    "SUBJECT",
    # DATE здесь НЕТ НАМЕРЕННО: маскирование дат отключено по договорённости с
    # заказчиком на уровне ВСЕХ слоёв (регэксп-детекторы DATE/DATE_RANGE не
    # зарегистрированы, LLM-детекция их не запрашивает) — см. рационале у
    # CORPORATE_DETECTORS в detectors.py. Раньше recall был единственным слоем,
    # который это решение нарушал: один и тот же документ давал то 18-20 масок
    # DATE, то 0 — недетерминированно, в обход согласованного поведения. Не
    # возвращать DATE в эту таксономию, не прочитав рационале в detectors.py.
})
...
_RECALL_TYPE_ALIASES: dict[str, str] = {
    ...
    # DATE-алиасов здесь больше нет: DATE исключён из _RECALL_LABELS (см.
    # комментарий там), а алиас на отсутствующую метку просто отбрасывался бы
    # в _normalize_recall_type — держать его дальше было бы вводящей в
    # заблуждение мёртвой записью.
    # BANK_ACCOUNT
    ...
    # Промпт recall-модели написан по-русски (см. _RECALL_SYSTEM_PROMPT) и
    # явно перечисляет допустимые типы латиницей, но реалистично модель всё
    # равно иногда отвечает русским названием типа («ФИО» вместо «PERSON») —
    # именно так в проде отбрасывались настоящие фамилии (см. комментарий у
    # _RECALL_LABELS/normalize и историю бага). Этот блок — страховка ПОВЕРХ
    # промпта, а не замена ему.
    # PERSON
    "ФИО": "PERSON", "ИМЯ": "PERSON", "ФАМИЛИЯ": "PERSON",
    "ОТЧЕСТВО": "PERSON", "ПЕРСОНА": "PERSON", "ЧЕЛОВЕК": "PERSON",
    # ORG
    "ОРГАНИЗАЦИЯ": "ORG", "КОМПАНИЯ": "ORG", "ЮРЛИЦО": "ORG", "БАНК": "ORG",
    # ADDRESS / LOCATION / STREET / HOUSE
    "АДРЕС": "ADDRESS",
    "ГОРОД": "LOCATION", "ЛОКАЦИЯ": "LOCATION", "МЕСТО": "LOCATION",
    "РЕГИОН": "LOCATION",
    "УЛИЦА": "STREET",
    "ДОМ": "HOUSE",
    # PHONE / EMAIL
    "ТЕЛЕФОН": "PHONE",
    "ПОЧТА": "EMAIL", "ЭЛЕКТРОННАЯ_ПОЧТА": "EMAIL", "ЭЛ_ПОЧТА": "EMAIL",
    # AMOUNT / URL (ДАТА сюда намеренно не отображается — см. _RECALL_LABELS)
    "СУММА": "AMOUNT", "ДЕНЬГИ": "AMOUNT", "ЦЕНА": "AMOUNT",
    "ССЫЛКА": "URL", "САЙТ": "URL",
    # PASSPORT / BANK_ACCOUNT / CONTRACT
    "ПАСПОРТ": "PASSPORT",
    "СЧЁТ": "BANK_ACCOUNT", "СЧЕТ": "BANK_ACCOUNT",
    "РАСЧЁТНЫЙ_СЧЁТ": "BANK_ACCOUNT", "РАСЧЕТНЫЙ_СЧЕТ": "BANK_ACCOUNT",
    "ДОГОВОР": "CONTRACT",
    # Аббревиатуры-коды: кириллическое написание визуально совпадает с
    # латинским лейблом, но это разные символы Unicode, поэтому без явного
    # алиаса не совпадёт ни с одним ключом _RECALL_LABELS.
    "ИНН": "INN", "ОГРН": "OGRN", "КПП": "KPP", "БИК": "BIK",
    "СНИЛС": "SNILS", "ОКПО": "OKPO",
}
...
# Список допустимых значений поля "type" для промпта — строится из
# _RECALL_LABELS программно (а не переписывается руками в промпте), чтобы
# промпт и таксономия НИКОГДА не могли разойтись: раньше промпт вообще не
# перечислял допустимые типы, из-за чего русскоязычная модель отвечала
# русскими названиями («ФИО» вместо «PERSON») и такие кандидаты целиком
# отбрасывались как «неизвестный тип» — включая настоящие фамилии. См.
# _RECALL_TYPE_ALIASES для страховки на случай, если модель всё же ответит
# по-русски вопреки этой инструкции.
_RECALL_TYPE_LIST = ", ".join(sorted(_RECALL_LABELS))
...
_RECALL_SYSTEM_PROMPT = (
    ...
    "(например [PERSON_1], [ORG_2], [PHONE_1], [BANK_ACCOUNT_1]). Найди значения, "
    ...
    "e-mail; номера и коды (счёт, ИНН, ОГРН, ОКПО, КПП, БИК, номер договора, "
    ...
    "слова, роли (Заказчик, Исполнитель, Сторона) и термины. НЕ включай даты, "
    "периоды и сроки (числа месяца, месяц/год, диапазоны дат, «до конца недели», "
    "«в течение 10 дней», дедлайны) — маскирование дат в этом документе "
    "намеренно отключено, даже если дата выглядит конфиденциально. Если "
    "сомневаешься, конфиденциально ли значение (и это НЕ дата) — ЛУЧШЕ включи "
    "его (перемаскировать безопаснее, чем пропустить).\n"
    "Сами токены вида [ТИП_НОМЕР] (например [PERSON_1], [ORG_2], [AMOUNT_3]) — это "
    ...
    "ДОПУСТИМЫЕ ЗНАЧЕНИЯ ПОЛЯ \"type\" — СТРОГО ОДНО из перечисленных ниже, "
    "заглавными латинскими буквами, ровно в этом написании (без перевода на "
    "русский и без собственных вариантов): "
    f"{_RECALL_TYPE_LIST}. Если значение не подходит ни под один из этих "
    "типов — НЕ включай его в ответ вовсе, даже если оно кажется "
    "конфиденциальным.\n"
    "Верни ТОЛЬКО JSON-массив объектов {\"text\": \"<точная подстрока из текста>\", "
    "\"type\": \"<ТИП строго из списка допустимых значений выше>\"}. Поле text "
    "должно ДОСЛОВНО (те же символы) совпадать с фрагментом текста. Если всё "
    "уже замаскировано — верни []. Без пояснений."
)
