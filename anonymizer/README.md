# anonymizer — обратимое маскирование (псевдонимизация) ПДн, русский язык

Ядро пайплайна маскирования: находит чувствительные данные, заменяет их на
уникальные плейсхолдеры (`[LABEL_N]`), сохраняет JSON-маппинг и умеет
**восстанавливать** исходный текст обратно по маппингу без ИИ.

> **Правовая оговорка.** Операция обратима, и таблица соответствий сохраняется,
> поэтому результат работы системы **не является обезличиванием** в значении
> п. 9 ст. 3 Федерального закона от 27.07.2006 № 152-ФЗ. Результат сохраняет
> статус персональных данных. Корректные термины для документации и переговоров:
> «маскирование», «псевдонимизация», «удаление прямых идентификаторов».
